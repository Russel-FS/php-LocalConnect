            ...
            <div class="max-w-4xl mx-auto p-8 lg:p-10">
                <form id="wizard-form" autocomplete="off">
                    ...
                    <div id="paso-6" class="hidden">
                        ...
                        <div id="resumen-negocio" class="space-y-6"></div>
                        <button class="px-2 bg-green-900 rounded-full text-white" type="button" onclick="mostrarResumen()">mostrar resumen</button>
                        <div class="flex justify-between mt-10">
...
<script>
    ...
    function mostrarResumen() {
        // 1. Datos del negocio
        const nombre = document.querySelector('input[placeholder="Ej: Panadería San Juan"]').value;
        const descripcion = document.querySelector('textarea[placeholder^="Describe tu negocio"]').value;
        ...
        // 3. Categorías
        const cats = Array.from(document.querySelectorAll('input[name="categorias[]"]:checked')).map(cb => cb.closest('label').querySelector('.font-medium').textContent);
        // 4. Servicios predefinidos
        const servs = Array.from(document.querySelectorAll('input[name="servicios_predefinidos[]"]:checked')).map(cb => cb.closest('label').querySelector('.font-semibold').textContent);
        // 5. Servicios personalizados
        const pers = Array.from(document.querySelectorAll('input[name^="servicio_personalizado"]')).map(inp => inp.value).filter(Boolean);
        // 6. Horario
        const dias = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'];
        let horarios = '';
        dias.forEach((dia, i) => {
            const cerrado = document.querySelector(`input[name='horarios[${i}][cerrado]']`).checked;
            const inicio = document.querySelector(`input[name='horarios[${i}][inicio]']`).value;
            const fin = document.querySelector(`input[name='horarios[${i}][fin]']`).value;
            horarios += `<tr><td class='py-1 px-2 font-medium text-gray-700'>${dia}</td><td class='py-1 px-2 text-xs'>${cerrado ? '<span class=\'text-red-500 font-semibold\'>Cerrado</span>' : `${inicio} - ${fin}`}</td></tr>`;
        });
        // 7. Contacto
        ...
        let mapaImg = '';
        if (lat && lng) {
            mapaImg = `<img src='https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/pin-s+2851e1(${lng},${lat})/${lng},${lat},16/400x200?access_token=pk.eyJ1IjoicnVzc2VsLWZzIiwiYSI6ImNtYTJ5djZ3NDFidzcybHNmZjl6dTEweGkifQ.L1_wuZGVMGSOmSKazwjxJg' class='w-full h-40 object-cover rounded-lg border mb-2' alt='Mapa' />`;
        }
        let html = `
        <div class="bg-white rounded-xl shadow p-6 flex flex-col sm:flex-row gap-6">
            ${img ? `<img src="${img}" class="w-32 h-32 object-cover rounded-lg border mb-4 sm:mb-0" alt="Portada" />` : ''}
            <div>
                <h4 class="font-bold text-xl mb-1">${nombre}</h4>
                <p class="text-gray-500 mb-2">${descripcion}</p>
                <div class="text-sm text-gray-400">${direccion}${distrito ? ', ' + distrito : ''}${ciudad ? ', ' + ciudad : ''}${provincia ? ', ' + provincia : ''}${departamento ? ', ' + departamento : ''}${pais ? ', ' + pais : ''}</div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow p-6">
            <h5 class="font-semibold text-lg mb-2 flex items-center gap-2"><svg class='w-5 h-5 text-primary-500' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-width='2' d='M7 7h.01M7 3h5a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z'/></svg> Categorías</h5>
            <div class="flex flex-wrap gap-2">
                ${cats.map(cat => `<span class="px-3 py-1 bg-primary-50 text-primary-700 rounded-full text-xs font-medium">${cat}</span>`).join('')}
            </div>
        </div>
        <div class="bg-white rounded-xl shadow p-6">
            <h5 class="font-semibold text-lg mb-2 flex items-center gap-2"><svg class='w-5 h-5 text-green-500' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-width='2' d='M9 12l2 2 4-4M21 12c0 4.97-4.03 9-9 9s-9-4.03-9-9 4.03-9 9-9 9 4.03 9 9z'/></svg> Servicios</h5>
            <div class="flex flex-wrap gap-2">
                ${servs.map(s => `<span class="px-3 py-1 bg-green-50 text-green-700 rounded-full text-xs font-medium">${s}</span>`).join('')}
                ${pers.map(s => `<span class="px-3 py-1 bg-yellow-50 text-yellow-700 rounded-full text-xs font-medium">${s}</span>`).join('')}
            </div>
        </div>
        <div class="bg-white rounded-xl shadow p-6">
            <h5 class="font-semibold text-lg mb-2 flex items-center gap-2"><svg class='w-5 h-5 text-blue-500' fill='none' stroke='currentColor' viewBox='0 0 24 24'><circle cx='12' cy='12' r='10' stroke='currentColor' stroke-width='2'/><path stroke-linecap='round' stroke-width='2' d='M12 8v4l3 3'/></svg> Horario de atención</h5>
        ...
        ${mapaImg ? `<div class='bg-white rounded-xl shadow p-6'><h5 class="font-semibold text-lg mb-2 flex items-center gap-2"><svg class='w-5 h-5 text-blue-400' fill='none' stroke='currentColor' viewBox='0 0 24 24'><circle cx='12' cy='12' r='10' stroke='currentColor' stroke-width='2'/></svg> Ubicación en el mapa</h5>${mapaImg}</div>` : ''}
        `;
        document.getElementById('resumen-negocio').innerHTML = html;
    }

    // Sobrescribe cambiarPaso para llamar mostrarResumen en el paso 6
