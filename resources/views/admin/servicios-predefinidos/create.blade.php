...
@section('content')
    <div class="max-w-lg mx-auto py-10">
        <h1 class="text-2xl font-bold text-primary-800 mb-6">Nuevo Servicio Predefinido</h1>
        <form action="{{ route('admin.servicios-predefinidos.store') }}" method="POST"
                ...
                <select name="id_categoria_servicio" class="w-full border rounded-lg px-4 py-2" required>
                    ...
                    @foreach ($categoriasServicio as $categoria)
                        <option value="{{ $categoria->id_categoria_servicio }}"
                            {{ old('id_categoria_servicio') == $categoria->id_categoria_servicio ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" name="nombre_servicio" class="w-full border rounded-lg px-4 py-2" required
                    maxlength="100" value="{{ old('nombre_servicio') }}">
            ...
            <div>
                ...
                <textarea name="descripcion" class="w-full border rounded-lg px-4 py-2" maxlength="255">{{ old('descripcion') }}</textarea>
                @error('descripcion')
                    <div class="text-red-600 text-xs mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit"
                    class="px-6 py-2 rounded bg-primary-600 text-white font-semibold hover:bg-primary-700">Guardar</button>
            </div>
        </form>
    </div>
@endsection
