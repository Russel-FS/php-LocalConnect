...
function mostrarResumen() {
    ...
    <div class="bg-white rounded-xl shadow p-6 flex flex-col sm:flex-row gap-6">
        ${
            img
                ? `<img src="${img}" class="w-32 h-32 object-cover rounded-lg border mb-4 sm:mb-0" alt="Portada" />`
                : ""
        }
        <div>
            <h4 class="font-bold text-xl mb-1">${nombre}</h4>
            <p class="text-gray-500 mb-2">${descripcion}</p>
            <div class="text-sm text-gray-400">${direccion}${
        distrito ? ", " + distrito : ""
    ...
    ${
        mapaImg
            ? `<div class='bg-white rounded-xl shadow p-6'><h5 class=\"font-semibold text-lg mb-2 flex items-center gap-2\"><svg class='w-5 h-5 text-blue-400' fill='none' stroke='currentColor' viewBox='0 0 24 24'><circle cx='12' cy='12' r='10' stroke='currentColor' stroke-width='2'/></svg> Ubicación en el mapa</h5>${mapaImg}</div>`
            : ""
    }
    `;
